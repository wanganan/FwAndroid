...
object StringUtils {
    ...
    fun htmlEscapeCharsToString(source: String): String? {
        return if (isEmpty(source)) source else source.replace(
            "&lt;".toRegex(),
            "<"
        )
            .replace("&gt;".toRegex(), ">")
    ...
    fun isEmpty(string: String?): Boolean {
        return string == null || "" == string.trim { it <= ' ' } || "null" == string.trim { it <= ' ' }
    }
    ...
    fun filterEmpty(string: String?): String? {
        return if (isNotEmpty(string)) string else ""
    }
    ...
    fun removeChar(string: String, index: Int): String? {
        ...
        val chars = string.toCharArray()
        result = if (index == 0) {
            String(chars, 1, chars.size - 1)
        } else if (index == chars.size - 1) {
            String(chars, 0, chars.size - 1)
        } else {
            String(chars, 0, index) +
                    String(chars, index + 1, chars.size - index)
        }
        return result
    }
    ...
    fun removeChar(string: String, index: Int, ch: Char): String? {
        var result: String? = null
        val chars = string.toCharArray()
        result = if (chars.size > 0 && chars[index] == ch) {
            if (index == 0) {
                String(chars, 1, chars.size - 1)
            } else if (index == chars.size - 1) {
                String(chars, 0, chars.size - 1)
